...
export async function getFeed(tab) {
  try {
    ...
    if (['article', 'youtube', 'podcast'].includes(tab)) {
      return mockAnnotations.filter((item) => item.source_type === tab);
    }
...
export async function getUserAnnotations(username, tab = 'annotations') {
  try {
    const data = await request(`/api/users/${username}/annotations`);
    const items = data.items || data.annotations || [];
    return tab === 'liked' ? items.slice().reverse() : items;
  } catch {
    return tab === 'liked' ? mockAnnotations.slice().reverse() : mockAnnotations;
  }
}
...
export async function togglePin(id) {
  return request(`/api/annotations/${id}/pin`, {
    method: 'POST',
    body: JSON.stringify({ user_id: CURRENT_USER_ID }),
  });
}
